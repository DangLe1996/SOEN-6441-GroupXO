...
public class GetTweets {
	...
	/**
	 * Store results of all searched queries in a formatted HTML string, using
	 * searched keyword as key. This GlobalCache reduce the needs to connect to
	 * Twitter4J if the term was already searched.
	 */
	...
	/**
	 * This method return a word-level statistics for the last 250 tweeets that
	 * contains a given SearchQuery, counting words in descending order by frequency
	 * of the words.
	 *
	 * @param SearchQuery
	 * @return  
	...
	public CompletionStage<Map<String, Integer>> GetKeywordStats(String SearchQuery) throws TwitterException {
		...
		query.count(250);
		query.lang("en");
		List<String> removewords = Arrays.asList("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n",
				"o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z", "as", "able", "about", "above", "according",
				"accordingly", "across", "actually", "after", "afterwards", "again", "against", "aint", "all", "allow",
				...
				"anywhere", "apart", "appear", "appreciate", "appropriate", "are", "arent", "around", "as", "aside",
				...
				"came", "can", "cant", "cannot", "cant", "cause", "causes", "certain", "certainly", "changes",
				...
				"sub", "such", "sup", "sure", "ts", "take", "taken", "tell", "tends", "th", "than", "thank", "thanks",
				"thanx", "that", "thats", "thats", "the", "their", "theirs", "them", "themselves", "then", "thence",
				"there", "theres", "thereafter", "thereby", "therefore", "therein", "theres", "thereupon", "these",
				"they", "theyd", "theyll", "theyre", "theyve", "think", "third", "this", "thorough", "thoroughly",
		...
		return CompletableFuture
				.completedFuture(twitter.search(query).getTweets().parallelStream().map(s -> s.getText().split(" "))
						// .map(s -> s.split(" "))
						.flatMap(Arrays::stream).map(t -> t.toLowerCase()).filter(t -> t.matches("[a-zA-Z]+"))
						.filter(t -> !removewords.contains(t)).collect(Collectors.toMap(s -> s, s -> 1, Integer::sum)));
	}

	/**
	 * Lambda BiFunction that takes a search query and its result, then return a
	 * formatted HTML string.
	 */
	private static final BiFunction<String, String, String> tweetDisplayPageFormat = (searchquery,
			result) -> "		<tr>\n" + "			<th>Search terms:</th>\n" + "			<th><a href=/keyword?s="
					+ searchquery.replaceAll(" ", "+") + ">" + searchquery + "</a></th>\n" + "	<th>"
					+ getTweetSentiments(searchquery) + "</th>" + "		</tr>\n" + "		<tr>\n"
					+ "			<th>User</th>\n" + "			<th>Location</th>\n"
					+ "			<th>Tweet Text</th>\n" + "		</tr>\n" + result;

	/**
	 * Returns sessionData contains searchQuery with given userID If GlobalCache
	 * contains a given keyword, then add it into LocalCache. If the words have not
	 * been looked up before, call the overloaded method of
	 * GetTweets_keyword(String) to look up the tweets, then add the result into
	 * GlobalCache and LocalCache of currentUser. If the given keyword length is
	 * less than 1, return the currentUser session data.
	 *
	...
	private BiFunction<QueryResult, String, QueryResult> formatSentimental = (result, keyword) -> {
		...
		analyse = result.getTweets().parallelStream().collect(groupingBy(s -> {
			if (stringContainsItemFromList(s.getText(), happy))
				return Mode.HAPPY;
			else if (stringContainsItemFromList(s.getText(), sad))
				return Mode.SAD;
			else
				return Mode.NEUTRAL;
		}));
	...
	/**
	 * Takes a string keyword and return a HTML formatted string showing the last 10
	 * tweets contains the given keyword. Only returns tweets with language of
	 * english.
	 *
	...
	public CompletionStage<String> GetTweets_keyword(String keyword) throws TwitterException {
		...
		if (GlobalCache.containsKey(keyword)) {
			...
			return CompletableFuture.completedFuture(GlobalCache.get(keyword));
		}
		...
		return invokeTwitterServer(query).thenApply(result -> formatSentimental.apply(result, keyword))
				.thenApply(result -> formatResult.apply(result)).thenApply(tweet -> {
	...
	/**
	 * Returns a boolean after matching a tweet text with provided list of happy/sad
	 * strings
	 * 
	...
	public static String renderSentimentsHTML(String mood, double percentage) {
		...
		String emoticon = "";
		if (mood.equals(Mode.HAPPY.toString()))
			emoticon = "\uD83D\uDE0C";
		else if (mood.equals(Mode.SAD.toString()))
			emoticon = "\uD83D\uDE1E";
		else
			emoticon = "\uD83D\uDE11";
...
//suhel ends
	...
	private Function<QueryResult, String> formatResult = (result) -> {
		try {
			...
			return result.getTweets().parallelStream().map(s -> {
				return "\n" + "<tr>\n" + "		<td><a href=/user?s=" + s.getUser().getScreenName().replaceAll(" ", "+")
						+ "> " + s.getUser().getScreenName() + "</a></td>\n" + "		<td><a href=/location?s="
						+ s.getUser().getLocation().replaceAll(" ", "+") + ">" + s.getUser().getLocation()
						+ "</a></td>\n" + "		<td>"
						+ s.getText().replaceAll("#(\\w+)+", "<a href=/hashtag?s=$1>#$1</a>") + "</td>\n" + "</tr>\n";
			}).limit(10) // suhel
	...
	private CompletableFuture<QueryResult> invokeTwitterServer(Query query) throws TwitterException {
